...
> **Amended 2026-08-28:** The breed/`Variety`/`Group` **taxonomy records are deferred**. ADR-0001 lists the taxonomy as declarative ruleset data, but the `Breed`/`Variety`/`Group` *data instances* and `breeds`/`varieties`/`groups` collections on `RulesetLayer` / `EffectiveRuleset` are **not** wired yet — no current consumer reads the records (the three policy ports operate on class/grade/award/placement and on the opaque `*Id` brands; `CollectiveCompetitionResults` carries `BreedId`/`VarietyId` as opaque references). The real readers are downstream contexts not yet built: catalogue ordering (`Group.ordinal`, `Breed.groupId`) in Catalogue & Publishing, and breed-recognition gating (`Breed.recognitionStatus`) in a future eligibility rule. The `BreedId`/`VarietyId`/`GroupId` brands and the `Breed`/`Variety`/`Group` **type definitions** in `breed.ts` remain as the published contract — they are not removed — and the dead `Breed`/`Variety`/`Group` *interfaces* are marked "deferred, not yet wired into a layer" so they are not mistaken for built features. Revisit when Catalogue & Publishing or a recognition-gating rule lands. See issue #134.
