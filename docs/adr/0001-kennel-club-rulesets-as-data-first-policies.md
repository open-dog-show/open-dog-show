...
> **Amended 2026-09-11 (#184):** three clarifications.
>
> - **Versioning.** The "Versioning" paragraph below is refined by [ADR-0029](0029-effective-ruleset-aggregate-resolved-from-layer-editions.md). The Effective Ruleset is an immutable aggregate referenced by id. It is resolved from dated **Ruleset Layer Editions** in force on the Show's date and may be re-pinned only until entries open.
> - **Rule changes are data; policy code is edition-agnostic.** A rule that differs between editions is expressed as edition data: a field on `AwardType`/`ClassDefinition`, or a new data parameter. Policy implementations never branch on a date or an edition. A change to policy code is therefore a bug fix or a new mechanism, and it applies to every Show, past and future. Results already recorded are protected because Judging stores them as facts.
> - **Where concrete rulesets live.** "Infrastructure only _loads_ the declarative data" is made literal. Authored edition content (FCI, SRSH/KMSH) lives in an infrastructure adapter behind a domain `RulesetLayerEditionRepository` port (`infrastructure/persistence/bundled/<ruleset>/`, one module per edition). Policy implementations stay in the domain (`domain/service/<ruleset>/`). The "pure domain modules" wording below now applies to the policies only.
