...
> **Amended 2026-08-26:** the `tenant` data-isolation unit was renamed to `club`
> throughout (issue #116) so the platform speaks consistently in the domain term
> `Club`. "multi-tenant"/"single-tenant deployment" read "multi-Club"/"single-Club
...
- **Deployment:** the **multi-Club hosted instance is the primary channel**; **single-Club** deployment (self-hosted, or hosted-by-us for one club) is a first-class supported mode — the same code run as one Club, fronted by the same thin connector. This preserves ADR-0001 (pure core) and ADR-0002 (multi-Club).
...
**4. Persistence — single PostgreSQL, schema-per-context, RLS multi-Club isolation, Drizzle.**
...
- Multi-Club isolation is **row-level: `club_id` + PostgreSQL Row-Level Security**, applied inside each context schema. Schema-per-Club and database-per-Club were rejected to avoid a contexts × Clubs explosion and to keep migrations single-pass. Single-Club deployments are simply one `club_id`.
...
- **Integration shape:** shipping the product _as_ a WordPress plugin (Model B) was rejected — it marries the framework (against ADR-0001), is inherently single-site (against ADR-0002's multi-Club isolation), and reaches only WordPress (~59% of CMS sites, a minority of all sites). The thin-connector hybrid recovers most of its one-click convenience without the concessions.
...
- **Open risk:** whether any club or National Canine Organisation will _require_ self-hosting / on-prem data is unquantified (research Q4). The supported single-Club deployment mode is the mitigation; a hard, widespread requirement could reopen the hosted-first emphasis.
