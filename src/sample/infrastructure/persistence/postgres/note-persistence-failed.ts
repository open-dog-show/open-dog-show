...
/**
 * Technical error thrown when the note Drizzle repository fails
 ...
 */
export class NotePersistenceFailed extends Error {
    constructor(operation: string, error: unknown) {
        super(`Note persistence failed while ${operation}`, {
            cause: error instanceof Error ? error.message : String(error),
        });
        this.name = 'NotePersistenceFailed';
    }
}
