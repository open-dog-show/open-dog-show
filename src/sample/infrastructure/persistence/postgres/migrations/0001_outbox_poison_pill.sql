...
-- before the handler runs — and `last_error` records the most recent failure
-- message; the dispatcher skips rows whose `attempts` have reached the
-- configured maximum, turning a poison pill into a skipped (quarantined) row
