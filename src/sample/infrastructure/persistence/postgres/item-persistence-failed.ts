...
/**
 * Technical error thrown when the item Drizzle repository fails
 ...
 */
export class ItemPersistenceFailed extends Error {
    constructor(operation: string, error: unknown) {
        super(`Item persistence failed while ${operation}`, {
            cause: error instanceof Error ? error.message : String(error),
        });
        this.name = 'ItemPersistenceFailed';
    }
}
