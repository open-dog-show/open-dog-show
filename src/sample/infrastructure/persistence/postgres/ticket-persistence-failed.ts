...
/**
 * Technical error thrown when the ticket Drizzle repository fails
 ...
 */
export class TicketPersistenceFailed extends Error {
    constructor(operation: string, error: unknown) {
        super(`Ticket persistence failed while ${operation}`, {
            cause: error instanceof Error ? error.message : String(error),
        });
        this.name = 'TicketPersistenceFailed';
    }
}
