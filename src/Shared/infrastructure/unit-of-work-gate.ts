...
export class UnitOfWorkGate {
     ...
     *   closed, given the aggregate's `typeName` and the name of the
     *   repository-port method that was called. Not narrowed to a literal
     *   union: the gate is shared kernel infrastructure, not owned by any
     *   one port shape — today's callers all wrap a `CrudRepositoryPort`
     *   (`'findById' | 'add' | 'update'`), but a future non-CRUD port's
     *   operation names are just as valid an argument. Kept as a callback
     *   rather than a fixed error type so this kernel class never imports a
