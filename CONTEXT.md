...
**Award Policy**:
The ruleset-owned rules that answer two questions for a given Award Scope Level and its results: (a) which Award Types may the Judge propose, and (b) are the Judge's proposed assignments valid? For example, the FCI Award Policy permits CACIB only when an Excellent-1st dog is present in a CACIB-eligible Class, and rejects a CACIB proposed for a dog with a lower grade. At the breed/group/show levels the policy is a generic **Feeder** matcher: a higher-scope Award Type is eligible only when at least one dog across its feeder **Candidate Streams** meets the Award's minimum Grade (and, at breed scope, a qualifying male and female are both present); a proposed dog must come from one of the Award's feeder streams and meet its minimum Grade. Stateless — evaluated once per scope, from that scope's placements or streams alone. Applies to individual class/scope judging (the four Award Scope Levels); Collective Competitions are governed by the **Collective Award Policy**. Part of the Rulesets' Published Language.
_Avoid_: Award Rules (too generic), Eligibility Rules (misses the validation half)

**Feeder**:
The award-or-class source that qualifies a Dog as a candidate for a higher-scope Award (ADR-0017). A Feeder is either an Award Type (e.g. CACIB feeds BOB; BIG feeds BIS) or a Class placement (e.g. the Puppy class 1st feeds Best Puppy in Show — the Puppy and Minor Puppy classes grant no Award Type, so the class win is the Feeder). Each higher-scope individual Award Type declares its Feeder(s) via `fedBy` on the Award Type; a multi-feeder such as BOB draws on the adult certificate plus the junior and veteran class wins. Authored per Ruleset Layer (last layer wins, wholesale replacement): the FCI base layer's BOB feeds on CACIB + class wins; a national layer (e.g. KMSH) overrides BOB to add the national CAC Feeder. Show-type awareness is by stream presence — out-of-scope Feeders match no stream, so no show-type parameter is needed. Cross-referenced from **Candidate Stream** / `fedBy`.
