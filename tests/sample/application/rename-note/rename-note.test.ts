...
import { NoteNotFoundError } from '../../../../src/sample/domain/model/note/note.js';
...
describe('RenameNoteHandler', () => {
    it('renames an existing Note', async () => {
        ...
        const response = await new RenameNoteHandler(unitOfWork).execute(
    ...
    it('throws NoteNotFoundError for an unknown id', async () => {
        ...
        await expect(
            new RenameNoteHandler(unitOfWork).execute(
