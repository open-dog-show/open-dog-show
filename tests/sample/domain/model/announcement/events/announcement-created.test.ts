...
import {
    ...
    AnnouncementCreated,
} from '../../../../../../src/sample/domain/model/announcement/events/announcement-created.js';

const AGGREGATE_ID = asAggregateId('00000000-0000-4000-8000-000000000021');

describe('AnnouncementCreated', () => {
    it('create constructs a fact with the fixed type and given scope/payload', () => {
        const event = AnnouncementCreated.create(AGGREGATE_ID, PlatformEventScope.of(), {
            name: 'A name',
        });

        expect(event.type).toBe(ANNOUNCEMENT_CREATED_TYPE);
    ...
    it('rehydrate reconstructs the same fact shape from stored fields', () => {
        const event = AnnouncementCreated.rehydrate({
            scope: PlatformEventScope.of(),
            aggregateId: AGGREGATE_ID,
            payload: { name: 'A name' },
        });

        expect(event).toBeInstanceOf(AnnouncementCreated);
        expect(event.payload).toEqual({ name: 'A name' });
    });
});
