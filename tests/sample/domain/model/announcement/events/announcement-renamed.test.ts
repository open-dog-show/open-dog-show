...
import {
    ...
    AnnouncementRenamed,
} from '../../../../../../src/sample/domain/model/announcement/events/announcement-renamed.js';

const AGGREGATE_ID = asAggregateId('00000000-0000-4000-8000-000000000021');

describe('AnnouncementRenamed', () => {
    it('create constructs a fact with the fixed type and given scope/payload', () => {
        const event = AnnouncementRenamed.create(AGGREGATE_ID, PlatformEventScope.of(), {
            name: 'A new name',
        });

        expect(event.type).toBe(ANNOUNCEMENT_RENAMED_TYPE);
    ...
    it('rehydrate reconstructs the same fact shape from stored fields', () => {
        const event = AnnouncementRenamed.rehydrate({
            scope: PlatformEventScope.of(),
            aggregateId: AGGREGATE_ID,
            payload: { name: 'A new name' },
        });

        expect(event).toBeInstanceOf(AnnouncementRenamed);
        expect(event.payload).toEqual({ name: 'A new name' });
    });
});
