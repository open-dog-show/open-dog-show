...
import {
    NOTE_CREATED_TYPE,
    NoteCreated,
} from '../../../../../../src/sample/domain/model/note/events/note-created.js';
...
describe('NoteCreated', () => {
    it('create constructs a fact with the fixed type and given scope/payload', () => {
        const event = NoteCreated.create(AGGREGATE_ID, PlatformEventScope.of(), {
            name: 'A name',
        });

        expect(event.type).toBe(NOTE_CREATED_TYPE);
    ...
    it('rehydrate reconstructs the same fact shape from stored fields', () => {
        const event = NoteCreated.rehydrate({
